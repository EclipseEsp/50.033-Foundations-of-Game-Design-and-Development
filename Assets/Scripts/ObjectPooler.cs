...
public class ObjectPooler : MonoBehaviour
{
    ...
    public static ObjectPooler SharedInstance; // static, so that it is easy to reference this object from other class
    ...
    void Awake()
    {
        ...
        foreach(ObjectPoolItem item in itemsToPool)
        {
            for (int i = 0; i < item.amount; i++)
            {
                ...
                GameObject enemy = (GameObject)Instantiate(item.prefab);
                enemy.SetActive(false);
                enemy.transform.parent = this.transform;
                ExistingPoolItem e = new ExistingPoolItem(enemy, item.type);
                pooledObjects.Add(e);
            }
        }
    }
